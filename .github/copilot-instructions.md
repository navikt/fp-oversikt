...
| Consumers          | `foreldrepengesoknad` (all logged-in apps) and `fp-soknad` (lookup)                 |
| Tech stack         | Standard fp Java backend using `fp-prosesstask`                                     |
| Main integrations  | `fp-sak`, SAF, PDL, Aa-register, Brreg, EREG                                        |
| Data               | PostgreSQL holding a cached view of upstream data (sak, applications, vedtak, docs) |

- Citizen-facing using TokenX. Authorization per endpoint using TilgangKontrollTjeneste.
...
Business REST-endpoints found in classes listed in `ApiConfig` method `getApplicationClasses()`. These are evolving.
