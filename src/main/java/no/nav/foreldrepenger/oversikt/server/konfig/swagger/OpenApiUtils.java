package no.nav.foreldrepenger.oversikt.server.konfig.swagger;
...
public record OpenApiUtils(SwaggerConfiguration swaggerConfiguration, Application application) {
    ...
    public static OpenApiUtils openApiConfigFor(String tittel, Application application) {
        var oas = new OpenAPI().openapi("3.1.1")
            .info(new Info().title(tittel).version(Optional.ofNullable(ENV.imageName()).orElse("1.0")).description("REST grensesnitt for Frontend."))
            .addServersItem(new Server().url(ENV.getProperty("context.path", "/fpoversikt")));
        var swaggerConfiguration = new SwaggerConfiguration().id(idFra(application))
            .openAPI(oas)
            .prettyPrint(true)
    ...
    public void buildOpenApiContext() {
        try {
            new JaxrsOpenApiContextBuilder<>().ctxId(idFra(application))
