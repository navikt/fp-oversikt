...
@ApplicationScoped
class EsPersonopplysningerDtoTjeneste {

    private static final Logger LOG = LoggerFactory.getLogger(EsPersonopplysningerDtoTjeneste.class);
    private static final long CACHE_ELEMENT_LIVE_TIME_MS = TimeUnit.MILLISECONDS.convert(60, TimeUnit.MINUTES);
    private static final LRUCache<String, EsPersonopplysningerDto> PERSONINFO_CACHE = new LRUCache<>(2000, CACHE_ELEMENT_LIVE_TIME_MS);

    private PdlKlient pdlKlient;
    private InnloggetBruker innloggetBruker;

    EsPersonopplysningerDtoTjeneste() {
        // CDI
    }

    @Inject
    EsPersonopplysningerDtoTjeneste(PdlKlient pdlKlient,
                                    InnloggetBruker innloggetBruker) {
        this.pdlKlient = pdlKlient;
        this.innloggetBruker = innloggetBruker;
    }

    EsPersonopplysningerDto forInnloggetPerson() {
        LOG.info("Henter engangsstønad personinfo for søker");
    ...
    private EsPersonopplysningerDto hentOgCachePersoninfo(Fødselsnummer søkersFnr) {
        var søker = hentSøker(søkersFnr.value());
        var personinfoDto = EsPersonopplysningerDtoMapper.tilDto(søker);
