...
class FpPersonopplysningerDtoTjeneste {
    ...
    private List<PersonMedIdent> hentBarnTilSøker(PersonMedIdent søker) {
        ...
        var dødfødteBarn = Stream.ofNullable(søker.person().getDoedfoedtBarn())
            ...
            .map(FpPersonopplysningerDtoTjeneste::dødfødtBarn)
            .map(person -> new PersonMedIdent(null, person))
            .toList();
        return Stream.concat(relaterteBarn.stream(), dødfødteBarn.stream()).filter(FpPersonopplysningerDtoTjeneste::barnErYngreEnn40Mnd).toList();
    }
    ...
    private static boolean harAdressebeskyttelse(Person person) {
